...
1. Use shpify app template remix as a jumping off point = https://github.com/Shopify/shopify-app-template-remix

2. App Extensions = https://shopify.dev/docs/apps/build/app-extensions

3. Checkout Analytics = https://shopify.dev/docs/api/checkout-ui-extensions/2025-04/apis/analytics

4. Marketing Automations = https://help.shopify.com/en/manual/promoting-marketing/create-marketing/marketing-automations/create

5. Shopify Web Pixels = https://shopify.dev/docs/api/web-pixels-api
...
This app includes a "Better Web Pixel Analytics" extension that tracks user behavior on the storefront and can be integrated with Google Analytics 4.

### Technical Requirements

1. **Access Scopes**: The app requires the following access scopes:
   - `write_pixels`: For creating and managing web pixels
   - `read_customer_events`: For accessing customer event data

2. **Customer Privacy Settings**: The web pixel must include privacy settings in the `shopify.extension.toml` file:
   ```toml
   [customer_privacy]
   analytics = true
   marketing = true
...
3. **Pixel Configuration**: The GA4 Account ID is configured in the app settings and passed to the web pixel extension.

### Web Pixel Events

The web pixel is set up to track the following events:
- `page_view`: Triggered when a customer views any page
- `product_viewed`: Triggered when a customer views a product page
- `collection_viewed`: Triggered when a customer views a collection page
- `search_submitted`: Triggered when a customer performs a search
- `cart_viewed`: Triggered when a customer views their cart
- `checkout_started`: Triggered when a customer begins checkout
- `checkout_completed`: Triggered when a customer completes a purchase

### Debugging

To verify the web pixel is functioning correctly:
1. Open your storefront or preview store
2. Open browser developer tools (F12 or right-click > Inspect)
3. Check the Console tab for messages starting with "BETTER ANALYTICS:"
4. Navigate between pages to trigger page_view events

### Troubleshooting

If web pixel events are not showing in the console:
1. Check that the app has the required access scopes in `shopify.app.toml`
2. Verify the customer privacy settings are properly configured in `extensions/better-web-pixle-analytics/shopify.extension.toml`
3. Make sure the web pixel shows as "Connected" in the Shopify admin under Settings > Customer events
4. Check that your browser doesn't have extensions blocking analytics or scripts
